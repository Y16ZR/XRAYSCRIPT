...
vtls="$(cat ~/log-install.txt | grep -w "Vless Ws Tls" | cut -d: -f2|sed 's/ //g')"
vnone="$(cat ~/log-install.txt | grep -w "Vless Ws None Tls" | cut -d: -f2|sed 's/ //g')"
tlsvl="$(cat ~/log-install.txt | grep -w "Trojan Grpc Tls" | cut -d: -f2|sed 's/ //g')"
...
xts="$(cat ~/log-install.txt | grep -w "Trojan Ws None Tls" | cut -d: -f2|sed 's/ //g')"

echo -e "\e[0;34m.-----------------------------------------.\e[0m"
echo -e "\e[0;34m|             \e[1;33mCHANGE PORT XRAY\e[m            \e[0;34m|\e[0m"
echo -e "\e[0;34m'-----------------------------------------'\e[0m"
echo -e " \e[1;31m>>\e[0m\e[0;32mChange Port For Xray :\e[0m"
echo -e "  [1]  Change Port Xray Vmess Ws, Vless Tcp Xtls & Trojan Grpc/Ws $tls"
...
sed -i "s/   - Xray Trojan Grpc Tls    : $tls/   - Xray Trojan Grpc Tls    : $tls1/g" /root/log-install.txt
